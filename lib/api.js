...
// Meta LLM API Request
export const getMetaLLMResponse = async (question) => {
  const apiKey = getDecryptedAPIKey("META_LLM_API_KEY");
  if (!apiKey) throw new Error("Missing Meta LLM API Key");

  try {
    const response = await axiosInstance.post(
      "https://api.metallama.com/v1/query",
      { query: question },
      {
    ...
    console.error("Meta LLM API Error:", error.response?.data || error.message);
    throw new Error("Failed to fetch Meta LLM response");
  }
};
