...
export const getOpenAIResponse = async (question, model, extractedText) => {
  ...
  if (model.startsWith("gpt-") || model === "o3-mini") {
    ...
    apiKey = localStorage.getItem("ollamaApiKey");
  }
  ...
  const requestPayload = {
    model: model.startsWith("ollama-") ? model.replace("ollama-", "") : model,
  ...
  let apiUrl =
    model.startsWith("gpt") || model === "o3-mini"
      ...
      : model.startsWith("llama") ||
        ...
        : model.startsWith("ollama")
          ? localStorage.getItem("ollamaModelsUrl") + "/api/chat"
  ...
  try {
    ...
    // If the response violates rules, remove the last user message
    if (assistantContent.includes("This request violates rules:")) {
      conversation.pop(); // remove last user question
    } else {
      // Otherwise append assistant response normally
      conversation.push({
        role: "assistant",
...
export const getGeminiResponse = async (
  ...
  try {
    ...
    requestDetails = {
      via: aiDefenseMode,
      method: "POST",
      url: `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${maskAPIKey(
        apiKey,
      )}`,
      headers: {
        "Content-Type": "application/json",
    ...
    // If the response violates rules, remove the last user message
    if (
      modelContent.includes("This request violates rules:") &&
      conversation.length > 0 &&
      conversation[conversation.length - 1].role === "user"
    ) {
      conversation.pop();
    } else {
      // Otherwise append the model response normally
      conversation.push({
        role: "model",
...
export const getBedrockResponse = async (question, model, extractedText) => {
  try {
    ...
    // If the response violates rules, remove the last user message
    if (
      assistantContent.includes("This request violates rules:") &&
      conversation.length > 0 &&
      conversation[conversation.length - 1].role === "user"
    ) {
      conversation.pop();
    } else {
      // Otherwise append the assistant response normally
      conversation.push({
        role: "assistant",
        content: assistantContent,
      });
